...
{
    public class MomRetailerImsCyDAL : ReturnData
    {
        ...
        public List<MomRetailerImsCyBEL> GetMomRetailerImsCy(string dCode, string rCode, string aCode, string tCode, string cCode)
        {
            try
            {
                ...
                DataTable dt = _dbHelper.GetDataTableWithAuditTrial(qry, "Retailer Wise IMS (Current Year - MOM)");
                Int64 cont = dt.Rows.Count;

                var item = (from DataRow row in dt.Rows
                            select new MomRetailerImsCyBEL
                            {
