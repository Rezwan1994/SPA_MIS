...
{
    public class DistBonusAdjProcessFinalizeDAL : ReturnData
    {
        ...
        public object GetProcessList()
        {
            try
            {
                string qry = " SELECT PROCESS_SLNO,PROCESS_NO, TO_CHAR(PROCESS_DATE,'DD/MM/RRRR')PROCESS_DATE, TO_CHAR(BONUS_START_DATE,'DD/MM/RRRR')BONUS_START_DATE, TO_CHAR(BONUS_END_DATE,'DD/MM/RRRR')BONUS_END_DATE, APPROVED_STATUS, TO_CHAR(APPROVED_DATE,'DD/MM/RRRR')APPROVED_DATE, PROCESS_RUN_STATUS, TO_CHAR(PROCESS_RUN_DATE,'DD/MM/RRRR')PROCESS_RUN_DATE" +
                             ...
                             " AND PROCESS_FINALIZE_STATUS='No'" +
        ...
        public object SearchData()
        {
            try
            {
                var qry = " select" +
                          ...
                          " PROCESS_FINALIZE_STATUS," +
                          " TO_CHAR(PROCESS_FINALIZE_DATE,'DD/MM/RRRR')PROCESS_FINALIZE_DATE" +
                          " FROM DIST_BONUS_PROCESS" +
                          " WHERE APPROVED_STATUS='Approved'" +
                          " AND PROCESS_RUN_STATUS='Yes'" +
                          " ORDER BY PROCESS_SLNO";
                ...
                var item = (from DataRow row in dt.Rows
                            select new DistBonusProcessBEL
                            {
                                ...
                                ProcessRunStatus = row["PROCESS_RUN_STATUS"].ToString(),
                                ProcessRunDate = row["PROCESS_RUN_DATE"].ToString(),
                                ProcessFinalizeStatus = row["PROCESS_FINALIZE_STATUS"].ToString(),
                                ProcessFinalizeDate = row["PROCESS_FINALIZE_DATE"].ToString()
                            }).ToList();
                return item;
        ...
        public bool UpdateDistBonusAdjProcessFinalize(DistBonusProcessBEL master)
        {
            try
            {
                IuMode = "U";
                string qry = " Update DIST_BONUS_PROCESS set PROCESS_FINALIZE_DATE=  to_date('" + master.ProcessFinalizeDate + "','dd/mm/rrrr') ,PROCESS_FINALIZE_STATUS='" + master.ProcessFinalizeStatus + "'" +
